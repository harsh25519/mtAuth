...
class EmailVerificationTokenRepositoryTest {
    ...
    private static final String USER_EMAIL_2 = "bob@example.com";
    ...
    void findByToken_WhenExists_ShouldReturnToken() {
        // Act
        Optional<EmailVerificationToken> found = tokenRepository.findByToken(VALID_TOKEN);
    ...
    void findByToken_WhenDoesNotExist_ShouldReturnEmpty() {
        // Act
        Optional<EmailVerificationToken> found = tokenRepository.findByToken(UNKNOWN_TOKEN);
    ...
    @Test
    @DisplayName("Should find the token associated with a specific User")
    void findByUser_WhenUserHasToken_ShouldReturnToken() {
        // Act
        Optional<EmailVerificationToken> found = tokenRepository.findByUser(userWithToken);

        // Assert
        assertThat(found).isPresent();
        assertThat(found.get().getToken()).isEqualTo(VALID_TOKEN);
    }

    @Test
    @DisplayName("Should return empty when the User exists but has no token")
    void findByUser_WhenUserHasNoToken_ShouldReturnEmpty() {
        // Act
        Optional<EmailVerificationToken> found = tokenRepository.findByUser(userWithoutToken);
    ...
    @Test
    @DisplayName("Should throw exception when saving without a required token string (Nullable = false)")
    void save_WhenTokenStringIsMissing_ShouldThrowException() {
        // Arrange
        EmailVerificationToken invalidToken = EmailVerificationToken.builder()
